...
{
    public class RecipeEditingManagerUnitTests
    {
...
Formę do pieczenia smarujemy masłem. Zarówno dno formy jaki i brzegi wykładamy papierem do pieczenia. Ciasteczka drobno kruszymy. Masło roztapiamy i łączymy z rozdrobnionymi ciasteczkami. Całość dokładnie mieszamy.", IsFavourite = false, IsTodaysRecipe = false, Ingredients = new List<Ingredient>()
        ...
        public void EditRecipe_WithProperRecipe_CanEditRecipe()
        {
            ...
            var manager = new RecipeEditingManager(mock.Object, new ConsoleWrapper(), new TagManager(new TagService(), new ConsoleWrapper()));
            //Act
        ...
        [Fact]
        public void EditRecipeProperties_WithProperRecipeAndSelectedSpecificProperty_ReturnsModifiedRecipe()
        {
            //Arrange
            ...
            mockConsole.SetupSequence(s => s.ReadLine())
                ...
                .Returns("n");
            var manager = new RecipeEditingManager(new RecipeService(), mockConsole.Object, new TagManager(new TagService(), mockConsole.Object));
            //Act
            var editedRecipe = manager.EditRecipeProperties(recipe);
            //Asser
            editedRecipe.Name.Should().Be(newRecipeName);
        }
        [Fact]
        public void EditRecipeIngredients_WithProperRecipeAndSpecificOperation_ReturnsRecipeWithModifiedIngredients()
        {
            //Arrange
            ...
            var manager = new RecipeEditingManager(new RecipeService(), mockConsole.Object, new TagManager(new TagService(), mockConsole.Object));
            //Act
            var editedRecipe = manager.EditRecipeIngredients(recipe);
            //Asser
